...
async fn handle_signal_message(msg: SignalMessage) {
    match msg {
        SignalMessage::OFFER { target_username, sdp, public_addr: peer_addr } => {
            // Someone wants to connect to us!
            info!("Received OFFER from {}: {}. Peer public addr: {:?}", target_username, sdp, peer_addr);
        ...
        SignalMessage::ANSWER { target_username, sdp, public_addr: peer_addr } => {
            // Host responded to our connection request!
            info!("Received ANSWER from {}: {}. Host public addr: {:?}", target_username, sdp, peer_addr);
...
pub unsafe extern "C" fn wavry_send_connect_request(username_ptr: *const c_char) -> i32 {
    if username_ptr.is_null() { return -1; }
