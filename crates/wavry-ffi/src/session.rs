...
// Imports
use wavry_media::{
    EncodeConfig, Codec, Resolution, 
    MacScreenEncoder, MacVideoRenderer, Renderer
};
use wavry_client::{run_client as run_rift_client, ClientConfig, RendererFactory};
use rift_core::{
    cc::{DeltaCC, DeltaConfig},
                ...
                match res {
                    Ok(frame) => {
                        if let Some(addr) = client_addr {
                             ...
                             // Ideally we chunk > MTU. 
                             // For now, let's just send header + data if < MTU, else drop (or chunk later)
                             // Actually, let's implement basic chunking here 
